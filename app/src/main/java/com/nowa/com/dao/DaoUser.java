...
public class DaoUser extends GeneralDao {
    ...
    public User getUser(String username, String password) {
        ...
        List<ParseObject> results = cloudQueries.getObject("_user", User.LOGIN, username);

        if(results != null && !results.isEmpty()) {
            ParseObject po = results.get(0);
            user = new User(po.getObjectId(), po.getString(User.LOGIN), po.getString(User.TOKEN),
                    po.getString(User.COURSE), po.getString(User.DESCRIPTION), po.getString(User.REGISTER_NUMBER),
                    po.getString(User.EMAIL));
        }
    ...
    public void getUserById(String idUser) {
        ...
        List<ParseObject> results = cloudQueries.getObject("_user", User._ID, idUser);
        ParseObject po = results.get(0);
